	...
	\begin{columns}
		\begin{column}{.5\textwidth}
				\begin{figure}
					...
					\onslide<3->{\input{Figures/Sod_time.tex}}
...
\begin{frame}[fragile]{Proper Orthogonal Decomposition}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item<6->\emph{Preprocessing}: Separating the spatial and temporal axis of the solution $f(x,v,t)$
			\begin{itemize}
...
\begin{frame}
	\begin{itemize}
		\item \emph{Outlook}:
		\begin{itemize}
			\item disentangle intrinsic variables
			\item train a model on various rarefaction levels
			\item evolve intrinsic variables in time with an LSTM
		\end{itemize}
		\item \emph{Discussion}:
		\begin{itemize}
			\item When does the training time and finding of hyperparameters pay up?
		\end{itemize}
	\end{itemize}
\end{frame}
\begin{frame}
