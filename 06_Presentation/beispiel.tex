	...
	\begin{column}{.5\textwidth}
		...
		\begin{equation}
			...
			size=title,title=transport,boxrule=0.4pt,colframe=blue,drop fuzzy shadow=green]{\partial_t f + v \partial_x f} =
			 \tcbhighmath[
			 size=title,title=BGK op.,boxrule=0.4pt,colframe=red,drop fuzzy shadow=red]{\frac{1}{\tau} (M_f - f)}
		\end{equation}}
	\end{column}
	\begin{column}{.5\textwidth}
...
\begin{columns}
	\begin{column}{.5\textwidth}
		\onslide<3->{\emph{Scale the duration to evolve into equilibrium}: relaxation time $\boldsymbol{\tau}$
		\begin{equation}
...
\begin{frame}[fragile]{Discretization in space and velocity space in 1D}
	\begin{itemize}
		\item \emph{Space and time discretization}:
		 $x_j = j\Delta x$ and $j\in \mathbb{Z}$, $v_k = k\Delta v$ and $k\in \mathbb{Z}$, $t^i=i\Delta t$ and $t\in \mathbb{N}$,
		\item \emph{Leads to:} set of ODE's in time 
			\begin{equation}
			\partial_t f_{j,k} = -(v_k)_1D_x f|_{j,k}(t) + \frac{1}{\tau}({M_f}_{j,k}(t) - f_{j,k}(t))\mathrm{.}
			\end{equation}
		\item \emph{$\mathbf{KJ}$ first-order differential equations}:\\
		$K$ gridpoints in space \& $J$ number of gridpoints in velocity space
		\item \emph{3D}:\\ $K^3J^3$ first-order differential equations
		\item \emph{Evaluation requires}: the moments of $f$.
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Proper Orthogonal Decomposition}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item $X=\input{Figures/Mat.tex}$ 
				%\only<8->{\qquad \qquad \input{Figures/singvec.tex}}
			\end{itemize}
			...
			\item<8->\emph{Eckard-Young Theorem}:
				\begin{equation}
					\underset{\tilde{X}, s.t. rank(\tilde{X})=r}{\operatorname{argmin}} \| 
					X -\tilde{X}\|_F
...
\begin{frame}[fragile]{Terminology}
	\begin{columns}
		\begin{column}{.5\textwidth}
			\begin{figure}
				\input{Figures/Sing_example.tex}
...
\begin{frame}[fragile]{Autoencoders}
	\begin{figure}
		\scalebox{.7}[.7]{\input{Figures/AE_scheme.tex}}
...
\begin{frame}[fragile]{Training}
	\begin{columns}
		\begin{column}{.5\textwidth}
			\begin{figure}
				\scalebox{1}[1]{\input{Figures/Sing_example_01.tex}}
...
\begin{frame}[fragile]{Concepts}
	\begin{columns}
		\begin{column}{.5\textwidth}
			\begin{figure}
				\scalebox{.5}[.5]{\input{Figures/Capacity.tex}}
...
\begin{frame}[fragile]{Number of intrinsic variables}
	\begin{figure}
		\centering
		\scalebox{.8}[.8]{\input{Figures/Var_iv.tex}}
...
\begin{frame}[fragile]{Time dependece of $L_2$-Error}
	\begin{columns}
		\begin{column}{.61\textwidth}
			\begin{figure}
				\scalebox{.7}[.7]{\input{Figures/ErrTime.tex}}
				\caption{
...
\begin{frame}[fragile]{A detailed look at reconstructions}
	\begin{columns}
		\begin{column}{.61\textwidth}
			\begin{figure}
				\vspace{-.5cm}
				\scalebox{.7}[.7]{\input{Figures/ErrWorst.tex}}
				\caption{
...
\begin{frame}[fragile]{Moments of $f$ and $\tilde{f}$}
	\begin{columns}
		\begin{column}{.61\textwidth}
			\begin{figure}
				\scalebox{.7}[.7]{\input{Figures/MacroError.tex}}
				\caption{
...
\begin{frame}[fragile]{Physical consistency}
		\begin{columns}
			\begin{column}{.61\textwidth}
				\begin{figure}
					\scalebox{.7}[.7]{\input{Figures/Conservation.tex}}
					\caption{
...
\begin{frame}[fragile]{Interpolation}
	...
	\end{table}
	\begin{figure}
		\scalebox{.7}[.7]{\input{Figures/all_int.tex}}
		\vspace{-.3em}
