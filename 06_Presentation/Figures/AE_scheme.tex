...
%rec enc
\draw[fill=BlueGreen,opacity=.5,thick,dashed,draw=blue] (-1,-1) rectangle  (6.3,3.5) ;
\draw [draw=none] (-1,3.5) -- (6.3,3.5) node [midway,below,draw=none,blue] 
{\Large Encoder: \(h(X)=\Phi\)};
%rec dec
\draw[fill=Red,opacity=.5,thick,dashed,draw=red] (13,3.5) rectangle  (5.7,-1) ;
\draw [draw=none] (13,3.5) -- (5.7,3.5) node [midway,below,draw=none,BrickRed] 
{\Large Decoder: g(\Phi)=$\tilde{X}$};
%%forpass
\draw[arrow,thick] (0,3.6)--(12,3.6)node [near start,above]{Forward Pass};
%bckpass
\draw[arrow,thick] (12,4.1)--(0,4.1)node [near start,above]{Backward Pass};

\draw [thick,decorate,decoration={brace,mirror, amplitude=3pt}] (-.3,-.3) -- (.3,-.3) node [midway, below = 4pt,draw=none] {Input Layer};
...
\draw [thick,decorate,decoration={brace,mirror, amplitude=3pt}] (5.7,0.45) -- (6.3,0.45) node [midway,below = 6pt,draw=none,fill=Yellow] 
{Code Layer};

\draw [thick,decorate,decoration={brace,mirror,amplitude=3pt}] (11.7,-.3) -- (12.3,-.3) node [midway,below = 4pt,draw=none,xshift=-.9ex] {Output Layer};
\end{tikzpicture}
