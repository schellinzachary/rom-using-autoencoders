...
The compression of the input data $I$ yields a code $C \in \mathbb{R}^{3x5000}$, composed  of the three instrinsic variables $c_0(x,t)$, $c_1(x,t)$ and $c_2(x,t)$. Each of them describes the transport of a discontinuity as seen in \cref{Fig:Code_Fully}. Hence the expolitability of the code in terms of constructing a ROM is not provided. The method of characteristics \Cref{labellist} provides a means to bypass this shortage. Intrinsic variable $c_i(x,t)$ must satisfy the conservative condition \cref{Eq. Mass_Const} and the transport equation \cref{Eq. Transport} with $i = 0,1,2$. \\
...
\begin{minipage}{.5\linewidth}
	...
	\end{equation}
\end{minipage}
The characteristics $u_i$ describe the constant transport velocities for each variable $c_i$ calculated using \cref{Eq. Characteristics}. Susequently enabling the usage of a simple plynomial interpoaltion of any degree. Furthermore a linear mapping \(A_ix_i=c_i\) can be applied for the construction of interpolated code variables.\\
\begin{equation}
