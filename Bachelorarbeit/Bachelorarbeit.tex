...
\\
For autoencoders the necessity to perform a transposition of the applied layers arises, which is straight forward for fully connected layers. Convolutional layers with strides greater than unity on the other hand  the transposition needs the calculation of the gradient w.r.t. the kernel as shown in \cref{labellist} \cite{Goodfellow}.
\begin{align}
	\mathsf{G}_{i,j,k} &= \frac{\partial}{\partial \mathsf{S}_{i,j,k}}J(\mathsf{S,K})\\
	g(\mathsf{G,I,s})_{i,j,k,l} &= \frac{\partial}{\partial \mathsf{K}_{i,j,k,l}}J(\mathsf{I},\mathsf{K}) = \sum_{m,n} \mathsf{G}_{i,m,n}\mathsf{I}_{j,(m-1)\times s+k,(n-1)\times s+l}\\
	h(\mathsf{K},\mathsf{G},s)_{i,j,k} &= \frac{\partial}{\partial \mathsf{I}_{i,j,k}}J(\mathsf{I},\mathsf{K}) = \sum_{\substack{l,m\\s.t\\(l-1)\times s+m=j}}\quad
												  \sum_{\substack{n,p\\s.t\\(n-1)\times s+p=k}}
												  \sum_q \mathsf{K}_{q,i,m,p}\mathsf{G}_{q,l,n} 
\end{align}
...
To start with a working model an educated guess is made about the initial design of the architecture. \Cref{Tab: First Guess} shows chosen hyperparameters  which are used.
...
Subsequently the activations in the layers will be studied. First five activations are applied to all the four layers. Second a combination of activation functions is studied, where the intrinsic variables will be activated with a different function than the remaining layers. After training the L2-Error\cref{labellist} will be evaluated on the unshuffled, complete dataset as described in \cref{Sec: Data Sampling}. Results can be observed in \cref{Tab:Activations}.
\begin{table}[!htbp]\centering
...
\begin{center}
	\begin{figure}[htbp!]
		\scalebox{.9}{\input{Figures/Layer_Sizes/Hydro/0.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Hydro/1.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Hydro/2.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Hydro/3.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Hydro/4.tex}}
...
\begin{center}
	\begin{figure}[htbp!]
		\scalebox{.9}{\input{Figures/Layer_Sizes/Rare/0.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Rare/1.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Rare/2.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Rare/3.tex}}
		\scalebox{.9}{\input{Figures/Layer_Sizes/Rare/4.tex}}
