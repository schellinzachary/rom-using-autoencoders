...
for i in range(3,8):
    ...
    for j in range(5):
        ...
        a = np.append(a,test_losses[-1])
        np.save(f'%s/Train_losses_lr{i}_{j}'%path,[train_losses,test_losses])

    np.savetxt(f'%s/learning_rate_evaluation'%path, a)
