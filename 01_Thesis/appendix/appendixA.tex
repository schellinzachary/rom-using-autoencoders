...
The finding of appropriate hyperparameters for the fully connected autoencoder is described here. The hyperparameters include number of layers i.e. depth, number of nodes per hidden layer i.e. width, batch size and non-linear activation functions.\\ 
To start with a working model first a guess about the some initial hyperparameters is done. These are summarize in \Cref{Tab:First Guess}.
\begin{table}[htp]
	\centering
	...
	\begin{tabular*}{15cm}{ @{\extracolsep{\fill}} c c c c c @{} }
		...
		\hline
		16 		&	3/5 &     2000&	    1e-4 &LeakyReLU\\
		\bottomrule
	\end{tabular*} \label{Tab:First Guess}
\end{table}
The experimentation design for finding the optimal number of layers i.e. depth is shown in \cref{Tab:Layer Size}. determined, as they set a consequential part of the representational capacity of the model and therefore can initiate over- and underfitting at an early stage of the parameter search. \cref{Fig:Layer_Size} and \cref{Fig:Layer Size Rare} in \cref{AppendixA} show the training and validation error for five designs shown in \cref{Tab:Layer Size}.\\
\begin{table}[htp]
	\centering
	\caption{www}
	\begin{tabular*}{15cm}{ @{\extracolsep{\fill}} c c c @{} }
		\toprule
		Number of layers &  Reduction per layer & Validation loss\\ [.5ex]
		\hline
		10 & 40, 40, 20, 10, 5, 3/5, 5, 10, 20, 40, 40 \\ \hline
		8 & 40, 40, 20 , 10, 3/5, 10, 20, 40, 40\\ \hline
		6 & 40, 40, 20 , 3/5, 20, 40, 40\\ \hline
		4 & 40, 40, 3/5, 40, 40\\ \hline
		2 & 40, 3/5, 40\\ \hline
