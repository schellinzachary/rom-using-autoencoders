...
\begin{table}[htp]
	\centering
	\caption{Results for the variation of activations in hidden-/code layers. Given is the minimum value of validation error as well as the the corresponding epoch and the \(\L2\) all evaluated with the best performing model.}
	\begin{tabular*}{15cm}{ @{\extracolsep{\fill}} c c c c c c c @{} }
		\toprule
		Activations & \multicolumn{2}{c}{Validation error} & \multicolumn{2}{c}{$\L2$} &\multicolumn{2}{c}{Epoch}\\ [.5ex]
		& \(\hy\)&\(\rare\)&\(\hy\)&\(\rare\)&\(\hy\)&\(\rare\)\\
		\hline
