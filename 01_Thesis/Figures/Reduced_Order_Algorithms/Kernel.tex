...
\begin{tikzpicture}[]
	%input
	\draw (-2,0) rectangle (0.5,1) node[midway]{\includegraphics[width=2.5cm,height=1cm]{Figures/Reduced_Order_Algorithms/ExamplesSOD-005.png}};
	%1stKernel
	...
	\draw [<-] (conv)--++(0,1.5cm)node[left]{\footnotesize Convolutional operation};
	%label flatteing operation
	\draw [<-] (flatten)--++(0,.7cm)node[left]{\footnotesize Flatten};
	%label fully connected operation
	\draw [] (1) -- (7) node[midway,above=-5pt] (fc){};
	\draw [<-] (fc)--++(0,1.5cm)node[left]{\footnotesize Fully conencted operation};
	%label kernel
	\draw [<-] (kernel)--++(0,1.7cm)node[left]{\footnotesize Kernel};
	...
	%label hidden stages
	\draw [thick,decorate,decoration={brace,mirror, amplitude=3pt}] (1.5,-.1) -- (7.55,-.1) node [midway,below = 6pt,draw=none] {\footnotesize 4 Hidden layers each with feature maps};
	%label hidden stages
	\draw [thick,decorate,decoration={brace,mirror, amplitude=3pt}] (8.5,-.1) -- (11.65,-.1) node [midway,below = 6pt,draw=none] {\footnotesize Flattened- \& code layer};
\end{tikzpicture}