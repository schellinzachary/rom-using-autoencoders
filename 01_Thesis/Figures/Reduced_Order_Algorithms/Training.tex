...
\tikzstyle{every node} = [font=\footnotesize]
	...
	\node [proc,join] (p1) {Forward propagation:\\ \(AE(\minib_i)=\tilde{\minib}_i\)};
	\node [proc,join] (p2) {Compute loss:\\ \(L(\minib_i,\tilde{\minib}_i)=E_{B_i}\)};
	\node [proc,join] (p3) {Backpropagation:\\ \(\nabla_{\frepar} J = \grd\) };
	\node [proc,join] (p4) {Optimize \(\frepar\):\\\(\frepar_{i+1} = \frepar_i + \Delta\frepar\)};
