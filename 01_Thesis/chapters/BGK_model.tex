...
\begin{align}\label{Eq:Conservation}
	\partial_t&\rho + \partial_x(\rho u) = 0 \mathrm{,}\\
	\partial_t&(\rho u) + \partial_x(\rho u^2 + p) = 0\mathrm{,}\\
	\partial_t&E + \partial_x(u(E+p)) = 0\mathrm{,}
\end{align}
...
\end{equation}
Here \(w_k\) are weights and is a vector \(\alpha(x,t)\) containing three elements from which a unique solution for can be determined \(\mathcal{M}_f\). Further details 
...
rarefaction KN
SOD\\
Physically speaking, highly rarefied gases exhibit a diffusion dominated behavior whereas with increasing compression, the advection behavior seizes gradually.
