...
Note, that \cref{Fig:SodTime} and \cref{Fig:SodTime0} is taken from \cite{Sod} in order to elaborate the general evolution in time of a gas of \(\Kn<0.01\) in Sod's shock tube. \Cref{Fig:ExamplesSod} shows solutions \(f(x,v,t_i)\) of the BGK model with \(t_0=0s\),\(t_1=0.06\) and \(t_3=0.12s\) for two rarefaction levels \(\Kn=0.00001\) and \(\Kn=0.01\). There the difference when increasing the dilution of a gas in Sod's shock tube is visible: An increased dilution leads to a smooth transition from region 1 to region 5 with the abundance of a pronounced shock front. 
\begin{figure}[H]
