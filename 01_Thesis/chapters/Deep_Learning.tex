...
\begin{figure}
	...
	\caption{\footnotesize Flowchart of the training process used in this thesis for an autoencoder. The data set \(\Pi\) for training is split up into a training \(\Pi_{train}\) and a validation \(\Pi_{val}\) set with a \(80/20\) ratio (a). The \(\Pi_{train}\) set is equally devided into \(N\) minibatches (b). Afterwards the weights are randomly initialized (c). A successive evaluation of the first minibatch \(AE(\Pi_{B_n})=\tilde{\Pi}_{B_n}\) takes place in (d). The mean squared error between \(\Pi_{B_n}\) and \(\tilde{\Pi}_{B_n}\) yields the error \(E_{B_n}\). Thereafter \(E_{B_n}\) is backpropagated through the network (f). The gradient is used to optimize weights \(w\) and biases \(b\) with \(Adam\) (g). Validation takes place in (h) with the evaluation of \(AE(\Pi_{val})=\tilde{\Pi}_{val}\), which yields the error \(E_{val}\) in (i). }
	\label{Fig:Training}
\end{figure}
