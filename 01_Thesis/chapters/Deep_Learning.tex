...
The selection of starting values for \(\frepar\) is of crucial importance for the success of any neural network to learn something useful in a reasonable time. The staring point strongly affects the time needed for the learning algorithm to converge or to converges at all. Moreover even if \(J(\frepar)\) reaches a minimum, different initial values can lead to a variety of generalization errors. Specifically the mechanism of how initial parameters affects generalization is not well understood. A selection of initial parameters can be beneficial considering the optimization task, but fail to benefit generalization. The topic of parameter initialization is only sketched out briefly in this thesis. For more details see \cite{Goodfellow} for an overview, \cite{LeCun98} for the default initialization in \texttt{pytorch} and \cite{he2015} for very deep networks with more than eight layers and rectifying non-linear activations.\\
Known is that the biases can be chosen to a heuristically constant value, while the weights need to be initialized so that they break symmetry between nodes. Imagine two hidden nodes with the same activation function connected to the same input. The same gradient would lead to a symmetric evolution during training, making one of them redundant. Hence weights are initialized randomly often drawn from a Gaussian distribution. It follows that the scale of the distribution scales the inital weights. Note that symmetry is broken stronger with large inital weights than with small initial weights. Considering that the SGD algorithm makes only small contributions to the initial weights we can observe a strong prior that the final weights should be close the their initial state. Now with this in mind, if we scale the distribution to produce large weights, we induce an additional prior, which states, that all nodes are connected and how they are connected. Small initial weights on the other hand induce a prior, that says that it is more likely for nodes to not be connected and that the learning algorithm can choose which nodes to connect and how strong. There are exist several heuristics for the scale of the initial weights. One can take into account for example the activation function of the layer as proposed in \cite{torch_ini} and scale the initial weights as needed. In this thesis we use the default initialization in \texttt{pytorch} for linear -and convolutional layers where we draw from \(U(-\frac{1}{\sqrt{m}},\frac{1}{\sqrt{m}})\) where \(m\) is the number of input nodes per layer as suggested in \cite{LeCun98}.

an updated version of the stochastic gradient descent algorithm (SGD) is used. SGD is derived from the common gradient decent method, where only a small portion of snapshots is used per iteration to compute the gradient for an update of \(\frepar\) in the opposite direction of the gradient. \\
\begin{algorithm}[H]
	\begin{algorithmic}
		\SetAlgoLined
