...
POD and the FCNN have different qualities. With the setting described above, POD uses up to 6.7 times more parameters than the FCNN for the given reconstruction losses. This suggests, that the quality of the parameters, that the FCNN has learnt surpasses those of POD. On the other hand, the FCNN shows to be limited in terms of reconstruction loss that can be achieved. Therefore, the applicability of either of the methods is decided by the use case. Whenever computational recources like available memory are scarce, then the FCNN is the method of choice. Other cases where reconstruction loss needs to be below that of the FCNN and computational resources are not limited, POD is the method of choice.\\
Why was the training of the CNN not successful, eventough it could benefit form using more parameters compared to the FCNN? Convolutional layers comprise sparsity through local connectivity of nodes as explained in \cite{Goodfellow}. This in turn yields a regulative effect. Additionally, convolutional layers have more hyperparameters that can be tuned than fully connected layers. This can make it harder to find the right set of hyperparameters for convolutional neural networks. Furthermore, the assumption, that the CNN only feeds on too less examples of the flows and therefore performs poorly, could not be validated. Augmenting the data through rotation and flipping did not succeed as expected. Therefore it is thought, that a combined effect of unfit hyperparameters and data scarcity result in the poor performance.       