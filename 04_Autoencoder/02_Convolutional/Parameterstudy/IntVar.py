...
class Encoder_2(nn.Module):
    def __init__(self, int_var=5):
        ...
        self.linearE1 = nn.Linear(in_features=128,out_features=self.int_var)
...
class Decoder_4(nn.Module):
    def __init__(self, int_var=5):
        ...
        self.int_var = int_var
        self.linearD1 = nn.Linear(in_features=self.int_var,
        out_features=192
        )
        self.convD1 = nn.ConvTranspose2d(
            64,32,(3,3),
...
for idx in range(2):
    for int_var in int_vars:
        ...
        if idx == 0:
            encoder = Encoder_2(int_var)
            decoder = Decoder_2(int_var)
        else:
            encoder = Encoder_4(int_var)
            decoder = Decoder_4(int_var)
        ...
        },'Results_intvar/noaugment/model{}-int{}-epoch{}-val_loss{:.3E}.pt'.format(
            idx,
            int_var,
            k_models[i,1],
            k_models[i,0]))
        ...
        torch.save({
            ...
            },'Results_intvar/noaugment/last-model-{}-int-{}.pt'.format(idx,int_var))
