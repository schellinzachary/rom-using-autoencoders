...
from scipy.interpolate import interp1d, Akima1DInterpolator, BarycentricInterpolator, PPoly, PchipInterpolator, KroghInterpolator, NearestNDInterpolator, LinearNDInterpolator, griddata
...
xi_new = np.linspace(1,5,num=5,endpoint=True)
I_1 = interp1d(x, z[:,0].detach().numpy(),kind='nearest')
I_2 = interp1d(x, z[:,1].detach().numpy(),kind='nearest')
I_3 = interp1d(x, z[:,2].detach().numpy(),kind = 'nearest')
I_4 = interp1d(x, z[:,3].detach().numpy(), kind = 'nearest')
I_5 = interp1d(x, z[:,4].detach().numpy(), kind = 'nearest')
...
# xv,yv=np.meshgrid(xi_new,x_new)
# xv1,yv1 = np.meshgrid(np.linspace(1,5,num=5,endpoint=True),np.linspace(1,2500,num=2500,endpoint=True),)

# I_ND = griddata(xv1,z.detach().numpy(), xv)
...
plt.plot(c[499],'-o''m',label='$Original$')
plt.plot(predict_new[499],'-v''k',label='$Prediction$')
plt.xlabel('$Velocity$')
plt.ylabel('$Probability$')
plt.legend()
plt.show()
